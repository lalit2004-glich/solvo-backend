  ...
  try {
    ...
    if (fetchError) {
      if (fetchError.code === 'PGRST116') {
        return NextResponse.json<AptitudeResponse>({
          success: true,
          result: null,
      ...
      console.error('[APTITUDE_FETCH_ERROR]', {
    ...
    const resultWithPercentage = result
      ? {
          ...result,
          percentage: Math.round((result.score_total / 30) * 100),
        }
      : null;

    return NextResponse.json<AptitudeResponse>({
      success: true,
      result: resultWithPercentage,
    });

  } catch (error) {
    console.error('[DASHBOARD_APTITUDE_ERROR]', {
      timestamp: new Date().toISOString(),
